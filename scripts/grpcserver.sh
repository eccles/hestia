...
if [ $# -lt 1 ]
then
	log_info "Must be 1 argument"
	exit 1
fi
if [ -z "${GOPACKAGE}" ]
then
	log_info "GOPACKAGE is undefind"
	log_info "This script must only be executed by 'go generate'"
	exit 1
fi
#
# Sort out all variations of gopackage regarding capitalisation.
UPPER=$(echo ${GOPACKAGE} | tr '[:lower:]' '[:upper:]' )
LOWER=$(echo ${GOPACKAGE} | tr '[:upper:]' '[:lower:]' )
FIRST=$(echo ${GOPACKAGE} | cut -c1 | tr '[:lower:]' '[:upper:]')
REST=$(echo ${GOPACKAGE} | cut -c2-| tr '[:upper:]' '[:lower:]' )
CAPITAL="${FIRST}${REST}"
...
// implments the ${LOWER}API.${CAPITAL}Server interfacw
type Service struct {
	${UNIMPLEMENTED}
